...
for %%1 in (C Z Y X W V U T S R Q P O N M L K J I H G F E D ) do if exist %%1:\FirPE echo %%1>Opath.txt
set /p Upath=<Opath.txt

echo %time% 过期插件包比对程序-使用%Upath%作为Edgeless盘符 >>X:\Users\Log.txt
echo %time% 过期插件包比对程序-获得文件列表 >>X:\Users\Log.txt
dir /b "%Upath%:\FirPE\Resource\*.*" >>X:\Users\Log.txt

dir /b "%Upath%:\FirPE\Resource\*.7z" >7zlist.txt
