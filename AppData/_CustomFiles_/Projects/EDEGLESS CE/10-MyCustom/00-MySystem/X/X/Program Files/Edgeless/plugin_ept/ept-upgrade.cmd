...
if not exist %EL_Part%:\FirPE\Resource\过期插件包 md %EL_Part%:\FirPE\Resource\过期插件包
if not exist %EL_Part%:\FirPE\Resource\过期插件包 (
...
for /f "usebackq delims==; tokens=*" %%i in ("X:\Users\ept\upgrade\UpgradeList_Path.txt") do (
    move /y "%%i" %EL_Part%:\FirPE\Resource\过期插件包 >nul
)
ren %EL_Part%:\FirPE\Resource\过期插件包\*.7z *.7zf
...
for /f "usebackq delims==; tokens=*" %%i in ("X:\Users\ept\upgrade\RenameList_FullName.txt") do (
    echo ept-upgrade 检查："%EL_Part%:\FirPE\Resource\%%i.7z" >>X:\Users\Log.txt
    if exist "%EL_Part%:\FirPE\Resource\%%i.7z" echo ept-upgrade 还原未升级的7zl：%%i >>X:\Users\Log.txt
...
for /f "usebackq delims==; tokens=*" %%i in ("X:\Users\ept\upgrade\7zlMatch.txt") do (
    echo ept-upgrade 检查："%EL_Part%:\FirPE\Resource\%%i" >>X:\Users\Log.txt
    if exist "%EL_Part%:\FirPE\Resource\%%i" echo ept-upgrade 还原升级的7zl：%%i >>X:\Users\Log.txt
